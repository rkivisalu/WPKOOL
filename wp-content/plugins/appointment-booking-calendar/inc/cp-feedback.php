<?php
add_action( 'admin_enqueue_scripts', 'cpabcal_feedback_insert_adminScripts', 1); 
add_action( 'wp_ajax_cpabcal_feedback', 'cpabcal_feedback' );
$cpabcal_plugfile =  'cpabc_appointments.php';
$cpabcal_plugslug = 'appointment-booking-calendar';
$cpabcal_supportlink = 'https://abc.dwbooster.com/contact-us';
$cpabcal_supportlink_full = $cpabcal_supportlink . '?priority-support=yes';

if (!get_option('installed_'.$cpabcal_plugslug, ''))
    update_option('installed_'.$cpabcal_plugslug, time() );

function cpabcal_feedback_insert_adminScripts($hook) {
    if( 'plugins.php' == $hook  )            
    {
        wp_enqueue_style('wp-jquery-ui-dialog');
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-dialog');       
        add_action( 'admin_footer', 'cpabcal_feedback_javascript' );            
    }
}
...
function cpabcal_feedback() {
    global $cpabcal_plugfile, $cpabcal_plugslug;
    $cpabcal_plugin_data = get_plugin_data( dirname(dirname(__FILE__)).'/'.$cpabcal_plugfile );
...
function cpabcal_feedback_javascript() { 
    global $cpabcal_plugslug, $cpabcal_supportlink, $cpabcal_supportlink_full; 
      ?>
...
$( window ).load(function() {
        document.querySelector('[data-slug="<?php echo $cpabcal_plugslug; ?>"] .deactivate a').addEventListener('click', function(event){
            ...
            $('<div title="QUICK FEEDBACK"><div style="padding:10px;">'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="no-work"> The plugin didn\'t work<br />'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="no-paypal"> Don\'t want to use PayPal<br />'+
                '<div id="abcm_nopaypal" style="margin-left:25px;display:none;padding:10px;border:1px dotted gray;color:#660000"><strong>No problem, we have a solution!</strong> There is a similar free plugin without PayPal, the <a href="plugin-install.php?tab=plugin-information&plugin=appointment-hour-booking&TB_iframe=true" class="thickbox open-plugin-details-modal" onclick="javascript:cpabcal_apphourbk(this);"><strong>Appointment Hour Booking</strong></a>. Questions? <a href="<?php echo $cpabcal_supportlink_full; ?>"><strong>Send us a message</strong></a> and get a solution.</div></div>'+
                                
                
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="-"> I don\'t like to share this information with you<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="temporary-deactivation"> It\'s a temporary deactivation. I\'m just debugging an issue.<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="better-plugin"> I found a better plugin<br />'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cpabcal_update_reason(this);" value="other"> Other<br />'+
      		   ...
      		   '</div></div>'
                  ...
                  buttons: [
                      ...
                      },
                      {
                        ...
                        click: function() {
                                   window.open('<?php echo $cpabcal_supportlink_full; ?>');
      						       $(this).dialog("close");
      				           }                            
                      },
                      {
...
function cpabcal_apphourbk(item)
{
    $("#abcm_nopaypal").closest('.ui-dialog-content').dialog('close');
    return true;
}

function cpabcal_update_reason(field)
{
    ...
    document.getElementById("abcm_nopaypal").style.display = 'none';
    if (field.value == '-')
    {
    ...
    else if (field.value == 'no-paypal')
        document.getElementById("abcm_nopaypal").style.display = ''; 
}
